...
// CLI set options
static int REUSE_SHM         = 0;
static int UNSIGNED_OUT      = 0;
static int BYTESHORTCAST     = 0;
static int STREAMNAMEINIT    = 0;
static int EMBED_MICROSECOND = 0;

static int PDV_UNIT = 0;
static int PDV_CHANNEL = 0; // same as cam
static char STREAM_NAME[200] = {0};
static int LOOPS = 1;
static int PDV_NUMBUFS = 4;
...
int main(int argc, char **argv)
{
    ...
    * and reads "pdv", which is what we want.
    */
    ...
    if((pdv_p = pdv_open_channel(EDT_INTERFACE, PDV_UNIT, PDV_CHANNEL)) == NULL)
    {
        sprintf(errstr,
    ...
    if(STREAMNAMEINIT == 0)
    {
        sprintf(STREAM_NAME, "edtcam%d%d", PDV_UNIT, PDV_CHANNEL);
    }

    if(REUSE_SHM)
    {
        ImageStreamIO_openIm(&image, STREAM_NAME);
    }
    else
    {
    ...
    const char  KW_TYPES[] = {'D', 'L', 'L', 'L', 'L', 'L'};
    const char *KW_COM[]   =
    {
        ...
        "Detected FG clock-per-line (pixel)",
        "Detected FG lines-per-frame (pixel)"
    };

    int KW_POS[] = {0, 1, 2, 3, 4, 5};
    ...
    image.kw[KW_POS[1]].value.numl = 0;
    image.kw[KW_POS[2]].value.numl = height;
    image.kw[KW_POS[3]].value.numl = isiowidth;
    image.kw[KW_POS[4]].value.numl = height; // initial val
    image.kw[KW_POS[5]].value.numl = isiowidth; // initial val

    /*
    * allocate four buffers for optimal pdv ring buffer pipeline (reduce if
    * memory is at a premium)
    */
    pdv_multibuf(pdv_p, PDV_NUMBUFS);
    ...
    */
    ...
    else
    {
        ...
        pdv_start_images(pdv_p, PDV_NUMBUFS);
    }
    ...
    */
    while(end_signaled == 0 && loopOK == 1)
    {
        ...
        */
        ...
        // Fetch the detected image size from the FG registers
        // this needs to be done before the continue when timing out.
        // This is useful for vampires auto-sizing!
        image.kw[KW_POS[4]].value.numl = edt_reg_read(pdv_p, PDV_CL_LINESPERFRAME);
        image.kw[KW_POS[5]].value.numl = edt_reg_read(pdv_p, PDV_CL_PIXELSPERLINE);
        ...
        */
        ...
        // This writes INTO the image... and therefore should obviously be after the memcopy... duh.
        if(EMBED_MICROSECOND)
        {
            ...
            *p_embed_acqtime = time_acq_micros;
            // Will cover 4 or 8 pixel whether type is int8 or int16
        }
        ...
        image.md[0].cnt1++;
        ImageStreamIO_UpdateIm_atime(&image, &time2);
